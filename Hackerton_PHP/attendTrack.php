<?php
  ...
  if (empty($sno)) {
    ...
    $query_mysql="SELECT * FROM TrackAttend WHERE ID = '" . $sno . "' and TrackKey = '" . $trackno . "' and
                                                    LectureKey = '" . $nodeno . "'";
    if ($result = mysqli_query($connect, $query_mysql)) {
      if (mysqli_num_rows($result) > 0) {
        ...
        mysqli_select_db($connect, "TrackAttend");
        $query_mysql="INSERT INTO TrackAttend(ID, TrackKey, LectureKey) VALUES('$sno', '$trackno', '$nodeno')";
        if ($result = mysqli_query($connect, $query_mysql)) {
          echo "Authorized";
        } else {
          echo "Unauthorized";
